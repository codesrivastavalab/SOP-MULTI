...
enum { NONE = 0, HARM, MORSE, LJSOP, REPSOP };
...
PairList_SOP::PairList_SOP(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 0;
  restartinfo = 0;
  respa_enable = 0;
...
void PairList_SOP::compute(int eflag, int vflag)
{
  ev_init(eflag,vflag);
  ...
  for (int n=0; n < npairs; ++n) {
    ...
    if (rsq < par.cutsq) {
      ...
      } else if (par.style == LJSOP) {

        const double r6inv = r2inv*r2inv*r2inv;
        const double sig6  = mypow(par.param.ljsop.sigma,6);
//        fpair =  24.0*par.param.ljsop.epsilon*r6inv
//          * (2.0*sig6*sig6*r6inv - sig6) * r2inv;
	fpair =  12.0*par.param.ljsop.epsilon*r6inv*r2inv
          * (sig6*sig6*r6inv - sig6) ;        

        if (eflag_either)
          epair = par.param.ljsop.epsilon*r6inv
            * (sig6*sig6*r6inv - 2.0*sig6) - par.offset;
      }else if (par.style == REPSOP) {

        const double r6inv = r2inv*r2inv*r2inv;
        const double sig6  = mypow(par.param.repsop.sigma,6);

	fpair =  6.0*par.param.repsop.epsilon*r6inv*r2inv*sig6;


        if (eflag_either)
          epair = par.param.repsop.epsilon*r6inv*sig6  - par.offset;
      }
...
void PairList_SOP::settings(int narg, char **arg)
{
  if (narg < 2)
  ...
  if (comm->me == 0) {
    ...
    try {
      ...
      while ((line = reader.next_line())) {
        ...
        case LJSOP:
          oneparam.param.ljsop.epsilon = values.next_double();
          oneparam.param.ljsop.sigma = values.next_double();
          ++nljsop;
          break;

        case REPSOP:
          oneparam.param.repsop.epsilon = values.next_double();
          oneparam.param.repsop.sigma = values.next_double();
          ++nrepsop;
          break;
    ...
    utils::logmesg(lmp, "Read {} ({}/{}/{}/{}) interacting pair lines from {}. "
                   "{} skipped entries.\n", npairs, nharm, nmorse, nljsop, nrepsop, arg[0], nskipped);
...
void PairList_SOP::coeff(int narg, char **arg)
{
  ...
  if (offset_flag) {
    for (int n=0; n < npairs; ++n) {
      ...
      if (par.style == HARM) {
        ...
        const double sig6  = mypow(par.param.ljsop.sigma,6);
        par.offset = par.param.ljsop.epsilon*r6inv * (sig6*sig6*r6inv - 2.0*sig6);
      }
	else if (par.style == REPSOP) {
        const double r6inv = par.cutsq*par.cutsq*par.cutsq;
        const double sig6  = mypow(par.param.repsop.sigma,6);
        par.offset = par.param.repsop.epsilon*r6inv *sig6;
      }      
    }
  }
}
...
double PairList_SOP::init_one(int, int)
{
  return cut_global;
}
...
double PairList_SOP::memory_usage()
{
